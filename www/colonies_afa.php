...
if ($response['success']) {
    $products = filter_products_by_category($response['data'], 'colonies-per-afa');
} else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['product_action'] ?? '') === 'edit_product') {
    ...
    $catId = category_id('descoberta', 'colonies-per-afa');
    if ($catId) {
    ...
    if ($update['success']) {
        flash('success', 'Colònia per afa actualitzada correctament');
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($_POST['product_action'])) {
    ...
    if ($create['success']) {
        flash('success', 'Colònia per afa creada correctament');
        redirect('/colonies_afa.php');
    } else {
...
<main class="content fade-in">
    <h1>Colònies per afa</h1>
    <p class="subtitle">Productes de WooCommerce amb la categoria colonies-per-afa</p>
    ...
    <div class="filters-card">
        <div class="filters-grid">
            <div class="filter-block">
                ...
                <input type="search" placeholder="Escriu el nom de la colònia per afa" data-table-search data-table-target="#activitats-table">
            </div>
        </div>
    </div>
