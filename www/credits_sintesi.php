...
if ($response['success']) {
    $products = filter_products_by_category($response['data'], 'credits-de-sintesi');
} else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['product_action'] ?? '') === 'edit_product') {
    ...
    $catId = category_id('descoberta', 'credits-de-sintesi');
    if ($catId) {
    ...
    if ($update['success']) {
        flash('success', 'Crèdit de síntesi actualitzat correctament');
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($_POST['product_action'])) {
    ...
    if ($create['success']) {
        flash('success', 'Crèdit de síntesi creat correctament');
        redirect('/credits_sintesi.php');
    } else {
...
<main class="content fade-in">
    <h1>Crèdits de síntesi</h1>
    <p class="subtitle">Productes de WooCommerce amb la categoria credits-de-sintesi</p>
    ...
    <div class="filters-card">
        <div class="filters-grid">
            <div class="filter-block">
                ...
                <input type="search" placeholder="Escriu el nom del crèdit de síntesi" data-table-search data-table-target="#activitats-table">
            </div>
        </div>
    </div>
    ...
    <div class="modal-overlay" id="modalEditActivitat">
        <div class="modal">
            <div class="modal-header">
                <h2>Editar crèdit de síntesi</h2>
