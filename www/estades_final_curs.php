...
if ($response['success']) {
    $products = filter_products_by_category($response['data'], 'estades-de-final-de-curs');
} else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['product_action'] ?? '') === 'toggle_status') {
    ...
    if (!$product) {
        ...
        redirect('/estades_final_curs.php');
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['product_action'] ?? '') === 'edit_product') {
    ...
    $catId = category_id('descoberta', 'estades-de-final-de-curs');
    if ($catId) {
    ...
    if ($update['success']) {
        flash('success', 'Estada de final de curs actualitzada correctament');
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($_POST['product_action'])) {
    ...
    if ($create['success']) {
        flash('success', 'Estada de final de curs creada correctament');
        redirect('/estades_final_curs.php');
    } else {
...
<main class="content fade-in">
    <h1>Estades de final de curs</h1>
    <p class="subtitle">Productes de WooCommerce amb la categoria estades-de-final-de-curs</p>
    ...
    <div class="filters-card">
        <div class="filters-grid">
            <div class="filter-block">
                ...
                <input type="search" placeholder="Escriu el nom de l'estada de final de curs" data-table-search data-table-target="#activitats-table">
            </div>
        </div>
    </div>
    ...
    <div class="modal-overlay" id="modalEditActivitat">
        <div class="modal">
            <div class="modal-header">
                <h2>Editar estada de final de curs</h2>
    ...
    <div class="modal-overlay" id="modalActivitat">
        <div class="modal">
            <div class="modal-header">
                <h2>Crear estada de final de curs</h2>
                <button class="modal-close" type="button">&times;</button>
            </div>
            <div class="modal-body">
                <form class="form-card" method="POST" enctype="multipart/form-data">
